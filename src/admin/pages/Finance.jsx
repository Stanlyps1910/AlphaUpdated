...
export default function Finance() {
    ...
    const fetchData = async () => {
        try {
            const [overviewRes, txRes, allocRes, pendingRes] = await Promise.all([
                axios.get(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/finance/overview`),
                axios.get(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/finance/transactions`),
                axios.get(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/finance/allocation`),
                axios.get(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/finance/pending-payments`)
            ]);
    ...
    const handleDeleteClick = async (id, type) => {
        ...
        try {
            if (type === 'income') {
                await axios.delete(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/invoices/${id}`);
                toast.success("Invoice deleted");
            } else {
                await axios.delete(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/finance/expense/${id}`);
        ...
        try {
            if (editExpense) {
                if (editExpense.type === 'income') {
                    await axios.put(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/invoices/${editExpense.id}`, {
                    ...
                    await axios.put(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/finance/expense/${editExpense.id}`, editExpense);
                    toast.success("Expense updated");
                }
            } else {
                await axios.post(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/finance/expense`, newExpense);
