...
export default function PhotographerProfile({ photographer, onClose, onUpdate }) {
    ...
    const fetchWorks = async () => {
        try {
            const response = await axios.get(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/photographers/${encodeURIComponent(photographer.name)}/works`);
    ...
    const handleSaveProfile = async () => {
        setLoading(true);
        try {
            const response = await axios.patch(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/photographers/${photographer._id}`, formData);
    ...
    const handleRemoveAssignment = async (workId, workPeople) => {
        ...
        try {
            ...
            await axios.patch(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/leads/${workId}`, { people: newPeople });
            fetchWorks();
                                                        ...
                                                        <button
                                                            onClick={async () => {
                                                                ...
                                                                try {
                                                                    ...
                                                                    await axios.patch(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/leads/${work._id}`, { people: newPeople });
