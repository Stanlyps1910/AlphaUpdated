...
export default function LeadDetails({ lead: initialLead, onClose, onGenerateInvoice }) {
  ...
  const handleUpdateNotes = async (notes) => {
    try {
      const response = await axios.patch(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/leads/${lead._id}`, { notes });
  ...
  const handleSaveLead = async () => {
    setSaving(true);
    try {
      const response = await axios.patch(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/leads/${lead._id}`, editData);
      setLead(response.data);
      setIsEditing(false);
  ...
  const fetchPhotographers = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/photographers`);
  ...
  const sendReminder = async (member) => {
    // e.stopPropagation() handled in onClick
    try {
      await axios.post(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/leads/${lead._id}/remind/${member}`);
  ...
  const handlePhotographerClick = async (member) => {
    ...
    if (photographer) {
      ...
      try {
        const res = await axios.get(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/photographers`);
