...
export default function TaskList({ leadId, tasks: initialTasks = [] }) {
  ...
  const addTask = async (e) => {
    ...
    try {
      let response;
      if (leadId) {
        response = await axios.post(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/leads/${leadId}/tasks`, {
          title: newTaskTitle,
          status: "pending"
        });
      } else {
        // Global task
        response = await axios.post(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/tasks`, {
          title: newTaskTitle,
          status: "pending"
        });
      }
  ...
  const toggleTask = async (taskId) => {
    try {
      const response = await axios.patch(`${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/tasks/${taskId}`);
